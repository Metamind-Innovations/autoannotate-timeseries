...
class AutoAnnotateGUI:
    ...
    def create_widgets(self):
        ...
        tk.Spinbox(
            frame, from_=128, to=2048, textvariable=self.context_length, width=10, font=("Arial", 10)
        ...
        self.progress = ttk.Progressbar(
            self.root, length=600, mode='indeterminate'
        )
    ...
    def browse_input(self):
        file = filedialog.askopenfilename(
            ...
            filetypes=[("CSV files", "*.csv"), ("TSV files", "*.tsv"), ("Parquet files", "*.parquet"), ("All files", "*.*")]
        )
        if file:
        ...
        try:
            ...
            if len(series_list) < self.n_clusters.get() * 3:
                response = messagebox.askyesno(
                    ...
                    f"You have {len(series_list)} time series but requested {self.n_clusters.get()} clusters.\n\n"
                    f"Recommended: At least {self.n_clusters.get() * 3} time series for good clustering.\n\n"
                    f"Continue anyway?"
                )
                if not response:
