...
class DatasetOrganizer:
    ...
    def create_split(
        ...
        for class_dir in class_dirs:
            ...
            timeseries_cols = [col for col in df.columns if col not in ['timestamp', 'time', 'date']]
        ...
        split_metadata = {
            "train_count": sum(info["n_series"] for info in split_info["train"].values()),
            "val_count": sum(info["n_series"] for info in split_info["val"].values()),
            "test_count": sum(info["n_series"] for info in split_info["test"].values()),
