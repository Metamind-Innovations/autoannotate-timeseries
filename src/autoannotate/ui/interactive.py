...
class InteractiveLabelingSession:
    ...
    def display_cluster_stats(self, stats: Dict):
        ...
        table.add_row("Unclustered Time Series", str(stats["n_noise"]))
        table.add_row("Total Time Series", str(stats["total_samples"]))
    ...
    def get_class_name_for_cluster(
        ...
        self.console.print(
            Panel(
                ...
                f"Cluster size: {cluster_size} time series\n"
                f"Representative samples: {len(representative_indices)} time series",
                style="green",
            )
        )
    ...
    def label_all_clusters_by_names(
        self,
        timeseries_list: List[np.ndarray],
        series_names: List[str],
        labels: np.ndarray,
    ...
    def display_labeling_summary(self, class_names: Dict[int, str], labels: np.ndarray):
        ...
        table.add_column("Time Series", style="yellow")

        for cluster_id, class_name in sorted(class_names.items()):
            n_timeseries = np.sum(labels == cluster_id)
            table.add_row(str(cluster_id), class_name, str(n_timeseries))
    ...
    def show_completion_message(self, output_dir: Path):
        ...
        self.console.print(
            Panel(
                ...
                f"Time series organized by class\n"
                f"Each class folder contains one CSV with all time series\n"
