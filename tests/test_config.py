...
class TestConstants:
    def test_supported_ts_formats(self):
    ...
    def test_model_configs_structure(self):
        ...
        assert "chronos-t5-tiny" in MODEL_CONFIGS
        assert "chronos-t5-small" in MODEL_CONFIGS
    ...
    def test_default_config_structure(self):
        ...
        assert "context_length" in DEFAULT_CONFIG["embedding"]
    ...
    def test_min_ts_per_cluster(self):
        assert isinstance(MIN_TS_PER_CLUSTER, int)
        assert MIN_TS_PER_CLUSTER > 0
    ...
    def test_default_ts_max_length(self):
        assert isinstance(DEFAULT_TS_MAX_LENGTH, int)
        assert DEFAULT_TS_MAX_LENGTH > 0

    def test_min_ts_length(self):
        assert isinstance(MIN_TS_LENGTH, int)
        assert MIN_TS_LENGTH > 0


class TestGetModelConfig:
    def test_get_chronos_tiny_config(self):
        config = get_model_config("chronos-t5-tiny")
        assert config["name"] == "amazon/chronos-t5-tiny"
        assert config["embedding_dim"] == 512

    def test_get_chronos_small_config(self):
        config = get_model_config("chronos-t5-small")
        assert config["name"] == "amazon/chronos-t5-small"
        assert config["embedding_dim"] == 512
    ...
    def test_config_is_copy(self):
        config1 = get_model_config("chronos-t5-tiny")
        config2 = get_model_config("chronos-t5-tiny")
